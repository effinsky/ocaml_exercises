...
let rle_modded lst =
  ...
  let rec aux count acc = function
    | [] ->
        []
    | [x] ->
        mk_tuple (count + 1) x :: acc
    | a :: (b :: _ as t) ->
        if a = b then aux (count + 1) acc t
        else aux 0 (mk_tuple (count + 1) a :: acc) t
  in
...
let replicate_items l count =
  ...
  let rec aux acc = function
    | [] ->
        acc
    | h :: t ->
        aux (acc @ create_partial count h) t
  in
...
let split_list_at (lst : 'a list) (at : int) : 'a list * 'a list =
  let rec aux idx (acc : 'a list * 'a list) = function
    ...
    | h :: t ->
        ...
        if idx < at then aux inc_idx (h :: fst, snd) t
        else aux inc_idx (fst, h :: snd) t
  in
